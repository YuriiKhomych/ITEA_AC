...
class MainApplicationLogicClass:
    ...
    def _handle_list_data(self, list_data, key=None, action=None,
        ...
        for data in list_data:
            if isinstance(data, dict):
                ...
                self._handle_list_data(data, key=key, collection=collection,
                                       action=action)
            else:
                self._handle_action(collection, key=key, value=data,
                                    action=action)
    ...
    def _handle_dict_data(self, dictionary, action=None, **collection):
        ...
        for key, val in temp_dict.items():
            ...
            if isinstance(val, dict):
                self._handle_dict_data(val, collection=collection,
                                       action=action)
            elif isinstance(val, list):
                self._handle_list_data(val, key=key, main_list=collection,
                                       action=action)
            else:
                self._handle_action(
    ...
    def _check_key(self, collection, key, action):
        if key in collection.keys():
            if ((key in self.keys_for_remove[1:7])
                or (key in [self.keys_for_remove[7][1],
                            self.keys_for_remove[8][1]]
                    and collection[key] != 'EUR')) and action == 'remove_key':
                self.remove_by_key(collection, key)
            if key in [self.keys_for_remove[7][0],
                       self.keys_for_remove[8][
    ...
    def _metric_sums_case_handler(self, list_comp):
        ...
        self._continuing_calculations(dict_sum, dict_sum_level,
                                      dict_sum_general, sum_funk=self.sum_funk,
            ...
            if 'api' in collection.keys():
                ...
                for data in collection['metric_sums']:
                    functions_mapping = {
                        ...
                        2: lambda: data['sum'] * data['sum_level'] ** 2
                            / 1 if data['sum_general'] == 0
                        else data['sum_general']
                            / 7 if period is None or period > 4 else period,
                        3: lambda: data['sum_level']
                            / 1 if data['sum_general'] == 0
                        else data['sum_general']
                            / 7 if period is None or period > 4 else period,
